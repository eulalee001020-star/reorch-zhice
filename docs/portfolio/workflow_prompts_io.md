...
质量门不是一个单独的 prompt，而是模型输出进入推荐前必须经过的验证链路：

| 判断项 | 标准 | 处理方式 |
| --- | --- | --- |
| 结构合法 | schema 通过 | 不通过则阻断 |
| 数据可追溯 | 结论能回到快照、工单、工序、设备 | 缺引用则降级为人工确认 |
| 硬约束 | 核心约束 100% 通过 | 不通过则不能推荐 |
| 业务风险 | 风险和取舍明确暴露 | 进入 warning 和人工确认 |
| 置信度 | 低置信度不自动预选 | 提示比较 Top-2 |
| 审计 | 全链路留痕 | 写入 decision/audit 记录 |

当前实现状态见 [trust_quality_gate.md](trust_quality_gate.md)。
