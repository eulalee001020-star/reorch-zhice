...
质量门不是为了让系统显得保守，而是为了把风险分层交给计划员判断。`block` 只用于硬约束失败、关键数据缺失、越权回写等不可接受场景；`warning` 可以进入候选列表，但必须解释风险且不能自动预选；`pass` 也只表示可进入人工确认，不代表自动执行。这样既保留工业安全边界，也避免计划员只看到一个被系统强推的“最优方案”。
