...
| 审计 | 推荐、确认、覆盖、回写、执行反馈均可留痕 | 核心链路已实现，生产审计包需继续增强 | `DecisionRecord`、`audit_metadata`、`AgentTraceStep`、`quality_gates`、`audit_logs`、confirmation/writeback records |
...
| 质量门仍需客户现场阈值校准 | 当前 pass/warning/block 已进入主流程和异常工作台，但阈值仍是 PoC 口径 | 用历史异常和计划员反馈校准不同车间的风险阈值 |
