...
| 模块 | 模型策略 | 成本控制 |
| --- | --- | --- |
| 异常分类、字段抽取 | 小模型或轻量分类器即可，低置信度进入人工确认 | 只传异常文本和必要上下文，不传完整排程表 |
| 规则候选结构化 | 中小模型 + schema 输出 | 结构化输出减少重试，候选规则不直接生效 |
| 方案解释 | 中等模型即可，必须基于结构化 KPI、质量门和 source refs | 常见解释模板化，复杂冲突场景再升级模型 |
...
当前 MVP 核心链路外部 LLM 调用为 0，数字孪生验证包通过 `model_cost_proxy` 记录外部调用数、估算 token、确定性步骤数、候选方案数和回写指令数。后续接入真实模型后，该结构会升级为真实 token、成本和 latency telemetry。
