...
目标岗位：AI 产品经理 / AI Agent 产品经理 / 工业智能产品经理
...
补充内容：这个项目的关键判断是把产品切口收窄到“异常响应层”，而不是直接挑战成熟 APS 的全厂级计划能力。这样更符合 B 端产品从单车间、单异常、高频痛点做付费 PoC 的落地规律。
...
补充内容：在工业场景，可信 AI 产品的关键不是“让模型更自主”，而是让模型在正确的责任边界内提高协同效率和解释效率。
...
作品集里有可运行前端 demo、后端 API、Docker Compose、测试、PoC 数据包、
prompt 工作流和商业化试点指标，展示的是一个 AI PM 从问题定义到落地验证的完整闭环。
```
