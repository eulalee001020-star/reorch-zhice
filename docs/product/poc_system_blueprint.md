...
目标：支撑已完成 MVP 的受控实验室演练，并为后续 4-6 周 Design Partner 试点做准备，而不是一次性完成全厂级 APS 替代。
...
现有代码已经具备异常接入、受控 Agent 工作流、影响分析、策略选择、求解、评价、解释、确认、反馈归因沉淀、案例库、初始调度、方案质量闸门、ROI 估算、数字孪生样例与验证证据包、adapter contract、Mock/CSV/REST 客户适配器、mock ERP/MES/APS server、PostgreSQL 优先持久化、登录/RBAC、后台任务、Docker Compose smoke CI。

下一阶段需要把 MVP 和数字孪生验证包现场化：

- 用真实客户数据验证 ERP/MES/APS 字段映射和接口协议，并从 mock server 切换到客户 sandbox。
...
- 在客户现场跑历史 replay，复核数字孪生 replay/shadow 代理指标是否符合计划员实际判断。
